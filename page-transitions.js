...
class PageTransition {
    ...
    initWorkScripts() {
        ...
        // Data for projects (kept in sync with work-script.js)
        const projects = [
            {
                number: '01',
        ...
        const techEl = document.querySelector('.project-tech');
        const detailsEl = document.querySelector('.project-details');

        // Replace any existing listeners on nav buttons
        ...
        function showVisual(type) {
            ...
            ids.forEach(id => {
                ...
                if (el) el.style.display = id.startsWith(type) ? 'block' : 'none';
            });
        }

        function updateActions(project) {
            const demoBtn = document.querySelector('.action-btn[title="Live Demo"], .action-btn[href="#"]');
            const githubBtn = document.querySelector('.action-btn[title="GitHub Repository"], .action-btn[target="_blank"]');
            const actionBtns = document.querySelectorAll('.project-actions .action-btn');
            if (actionBtns.length >= 2) {
                const first = actionBtns[0];
                const second = actionBtns[1];
                // First is live demo
                if (first.tagName === 'A') {
                    first.href = project.demo;
                    first.title = project.demo !== '#' ? 'Live Demo' : 'Demo Coming Soon';
                }
                // Second is github
                if (second.tagName === 'A') {
                    second.href = project.github;
                    second.title = project.github !== '#' ? 'GitHub Repository' : 'Repository Private';
                }
            }
        }
        ...
        function updateProject(index) {
            ...
            if (!project || !detailsEl) return;

            // Animate out
            detailsEl.style.opacity = '0';
            detailsEl.style.transform = 'translateX(-20px)';
            ...
            setTimeout(() => {
                ...
                // Visual + Actions
                showVisual(project.visual);
                updateActions(project);

                // Animate in
                detailsEl.style.opacity = '1';
                detailsEl.style.transform = 'translateX(0)';
            }, 200);
        }
